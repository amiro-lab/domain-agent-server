...
## 타입 정의 (반드시 구분)
- **fact**: 객관 사실·현황·이벤트. 관찰/측정/선언된 단일 사실, 진행 상황, 날짜, 수치, 누가 무엇을 담당하는지 등. "지금 이런 상태/이런 일이 있었다".
- **preference**: 원칙·전략·선호·정책. **Why(이유)와 How to apply(적용 시점)를 본문에 포함**할 것. "이런 이유로 이런 방식을 선호/금지한다".
- **ontology**: 도메인 용어·개념·스키마·분류 체계. 도메인 어휘 사전 역할. "X라는 용어는 정확히 무엇을 의미한다 / Y는 A·B·C로 나뉜다 / 이 시스템은 이런 구조다".
